...
export function RegisterForm() {
  ...
  const getPasswordStrength = () => {
    if (!password) return { label: "", color: "bg-slate-200 dark:bg-slate-800", width: "w-0" }
    if (password.length < 6) return { label: "Weak", color: "bg-red-500", width: "w-1/3" }
    if (password.length < 10 || !/\d/.test(password)) return { label: "Medium", color: "bg-yellow-500", width: "w-2/3" }
    ...
    <div className="w-full max-w-md mx-auto bg-white dark:bg-card rounded-2xl shadow-xl border border-border overflow-hidden">
      <div className="p-8 sm:p-10">
        <div className="flex flex-col items-center justify-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-2">Get started with SaaS Kit today</p>
        </div>
          ...
          <div className="space-y-2">
            ...
            {password && (
              <div className="mt-2 flex items-center space-x-2">
                ...
                <span className={`text-xs font-medium ${strength.label === 'Weak' ? 'text-red-500' : strength.label === 'Medium' ? 'text-yellow-500' : 'text-green-500'}`}>
                  {strength.label}
                </span>
              </div>
            )}
        ...
        <div className="grid grid-cols-2 gap-4">
          <Button variant="outline" type="button" onClick={() => handleOAuth('google')} className="h-11 rounded-xl bg-white dark:bg-card hover:bg-slate-50 dark:hover:bg-slate-900 border-border shadow-sm">
          ...
          <Button variant="outline" type="button" onClick={() => handleOAuth('github')} className="h-11 rounded-xl bg-white dark:bg-card hover:bg-slate-50 dark:hover:bg-slate-900 border-border shadow-sm">
